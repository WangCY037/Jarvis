...
from jarvis.memory.memory import MessageType
from xiaogpt.bot import get_bot
from jarvis.configs.Config import (
    COOKIE_TEMPLATE,
    LATEST_ASK_API,
...
class MiGPT:
    def __init__(self, config: Config, client_session: Optional[ClientSession] = None, bot = None):
        ...
        self._should_close_session = client_session is None

    async def close(self):
        """只在我们创建了session的情况下关闭它"""
        if self._should_close_session and not self.mi_session.closed:
            await self.mi_session.close()

    async def poll_latest_ask(self):
        """修改轮询方法，使用已存在的session"""
    ...
    async def run_forever(self):
        ...
        self.log.info(f"Running xiaogpt now, 用{'/'.join(self.config.keyword)}开头来提问")
        self.log.info(f"或用{self.config.start_conversation}开始持续对话")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    self.log.error(f"{self.chatbot.name} 回答出错 {str(e)}")
                else:
                    self.log.info("回答完毕")
                if self.in_conversation:
                    self.log.info(f"继续对话, 或用`{self.config.end_conversation}`结束对话")
                    await self.wakeup_xiaoai()
        finally:
            task.cancel()
