...
from xiaogpt.mi_utils import micli
...
class MiGPT:
    ...
    def need_ask_gpt(self, record):
        ...
        return (
            ...
            or (not query.startswith(WAKEUP_KEYWORD) or query.startswith(tuple(self.config.keyword))) )
                ...
                try:
                    if not self.config.enable_edge_tts:
                        ...
                        async for message in self.ask_gpt(query):
                            message = re.sub("A", "", message)
                            if "B" in message:
                                new_message = re.sub("B(.+)", "", message)
                                order = re.findall("B(.+)", message)[0]
                                message = new_message
                                print("order",order)
                            await self.do_tts(message, wait_for_finish=True)
                        #os.system(f"micli.py 5-4 {order} #1")
                        await miio_command(
                            self.miio_service,
                            self.config.mi_did,
                            f"5-4 {order} #1")
